...
def appendChecksum(packet):
	...
	for i in packet[1:]:
		checksum += i
	packet.append(int(math.floor(checksum % 256)))

packetBuffer = 500
lastPacketSent = time.time()
def sendPacket(packet):
	global lastPacketSent, packetBuffer
	timeSince = time.time() - lastPacketSent
	if (timeSince * 1000 < packetBuffer):
		return
...
def subscriber_callback(data):
	...
	if (mode == 0): # right arm
		...
		x = (data.axes[0] + 1.0) / (math.pi * 2) * 65535
		...
		packet.append(6)
		packet.append(0x11)
		packet.append(int(math.floor(x % 256)))
		packet.append(int(math.floor(x / 256)))
